...
> **Status**: In Progress
> **Created**: 2026-06-29
> **Last Updated**: 2026-06-30 (RD-06 essential controls + validators **drafted**; RD-11 containers/scrolling/lists **stubbed** as its sibling — split per AR-93)
...
| RD-06 | Essential controls + validators | [RD-06](requirements/RD-06-essential-controls.md) | — | RD Drafted | ✏️ | 2026-06-30 | Phase 1. **Drafted** ([RD](requirements/RD-06-essential-controls.md)) — the focused **leaf controls + validators** (split per AR-93): `Text`/`Label`/`Button`/`Input`/`CheckGroup`/`RadioGroup` (+ internal `Cluster` base) + the validator model `filter`/`range`/`lookup`. Adds the faithful **`cpGrayDialog` control theme roles** to core `Theme` (staticText/label*/button*/cluster*/input*, slots 6–21/31 — the one cross-package edit). New `packages/ui/src/controls/` subsystem. Demo: headless `demo:controls`. 13 AC; AR-93…AR-102 (7 user + 3 dominant/source). **Deferred (tracked) → RD-07:** Input selection+clipboard (AR-94), `picture`/mask validator (AR-95), `MultiCheckGroup` (AR-96). `ScrollBar`/`Scroller`/`ListView`/`Dialog` → **RD-11**. Next: `make_plan RD-06`. |
