...
> **Status**: In Progress
> **Created**: 2026-06-29
> **Last Updated**: 2026-07-01 (RD-06 essential controls + validators **planned** → `plans/essential-controls/`; RD-11 stubbed as its sibling)
> **Progress**: 6 / 11 done (RD-01 ✅, RD-02 ✅, RD-03 ✅, RD-04 ✅, RD-05 ✅, RD-10 ✅) · RD-06 ✏️ drafted
...
| RD-06 | Essential controls + validators | [RD-06](requirements/RD-06-essential-controls.md) | [essential-controls](plans/essential-controls/00-index.md) | Plan Created | 📋 | 2026-07-01 | Phase 1. **Planned** ([plan](plans/essential-controls/00-index.md)) — the focused **leaf controls + validators** (split per AR-93): `Text`/`Label`/`Button`/`Input`/`CheckGroup`/`RadioGroup` (+ internal `Cluster` base) + `filter`/`range`/`lookup`. Adds the faithful **`cpGrayDialog` control theme roles** to core `Theme` (the one cross-package edit; buttons reuse `button`/`buttonFocused`) + one additive intra-ui primitive (`ev.emit`/`ev.focusView` on the dispatch envelope). New `packages/ui/src/controls/` subsystem; headless `demo:controls`. **Plan:** 7 phases / 20 sessions / 24 tasks / ~21–35 h, spec-first (ST-01…ST-16); PA-1…PA-14 (3 user + 11 dominant/source); grounded in 3 recon passes (UI seams · TV `t*.cpp` geometry · `tvalidat.cpp` + `app.h` palette, hand-verified). 13 AC; AR-93…AR-102. **Deferred (tracked):** Input selection+clipboard/`picture`/`MultiCheckGroup` → RD-07; modal focus-trap → RD-11; multi-column cluster + Text-align (DEF-16…18). `ScrollBar`/`Scroller`/`ListView`/`Dialog` → **RD-11**. Next: `exec_plan essential-controls`. |
| RD-11 | Containers, scrolling & lists | [RD-11](requirements/RD-11-containers-scrolling-lists.md) | — | Backlog (stub) | 🟡 | 2026-06-30 | Phase 1 sibling of RD-06 (split per AR-93). **Stub** — scope reserved: `ScrollBar`/`Scroller`/`ListView` (+ `ListBox`) + the rich **`Dialog`** (hosts RD-06 controls via `execView`; adds the ScrollBar 4–5 + ListViewer 26–29 cpDialog roles). Authored fully via `add_requirement` once RD-06's control shape is settled. |
...
- **Recommended next:** **`exec_plan essential-controls`** — execute spec-first (per control: spec
  oracles RED → implement → GREEN → impl tests, reading each TV source first); optionally `preflight` the
  plan first. RD-11 is authored via a follow-up `add_requirement` once RD-06's control shape is settled.
