...
- **RT-1 (runtime):** ST-9 (override `mouse.sgr:false` over a terminal whose mouse drag/wheel are detected as on, per the testing strategy in 07) was not assigned to a specific session in 99-execution-plan.md (1.1.1 lists ST-1…ST-8/ST-10/ST-12; 2.1.1 lists ST-11/ST-17/ST-18/ST-19). It depends on the layer-4 table to produce a terminal with drag/wheel on, which first exists in Phase 2. **Decision:** author ST-9 in the Phase 2 spec-test session (2.1.1) alongside ST-18, where the table is available. No ambiguity in expected behaviour — PL-7 deep-merge already governs it.
- **RT-2 (runtime):** 03-01 defines `resolveCapabilities(options?)` returning a synchronous `CapabilityResolution`, with `query`/`timeoutMs` inside `ResolveOptions`. But layer 2 (`runQueries`) is inherently async (it awaits stream bytes under a `Promise.race` timeout), and ST-13/15/16 require passing a `query` to *resolve* and having the whole resolution complete within the timeout and fall back. A single function that is sync without a query and async with one is the "release Zalgo" anti-pattern. **Decision (user-confirmed 2026-06-27):** split into two entry points — `resolveCapabilities(options)` stays **synchronous** (layers 1/3/4/5; usage examples unchanged) and a new `resolveCapabilitiesAsync(options)` returns `Promise<CapabilityResolution>` and runs layer 2 before delegating to the same compose→override→freeze core. The **sync** signature is type-narrowed to `Omit<ResolveOptions, 'query' | 'timeoutMs'>` so passing a query to the sync path is a compile error; `ResolveOptions` remains the full async superset. Phase 3 spec tests target `resolveCapabilitiesAsync` + `runQueries`. Mirrors Node's `readFileSync`/`readFile` convention.
