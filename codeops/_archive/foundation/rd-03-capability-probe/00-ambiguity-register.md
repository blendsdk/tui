...
| RT-6 | Technical (runtime) | `--auto` live query vs stdout JSON | run the live query over stdout (pollutes the JSON) / use a null query (env/table facts only) | **`--auto` uses a null `TerminalQuery` (write no-op, read yields nothing) so query escape sequences never reach stdout, which carries the JSON report. Auto facts come from env/table resolution — correct for headless CI (RD AC-5). Live queries remain interactive-only.** | ✅ Resolved |
