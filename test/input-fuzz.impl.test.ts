...
test('randomBytes(seed) is reproducible and in range 0x00–0xff', () => {
  ...
  assert.deepEqual(Array.from(bytes), Array.from(randomBytes(mulberry32(525600), 32)));
  for (const b of bytes) assert.ok(b >= 0 && b <= 255);
});
...
test('pinned case: an unterminated paste flood stays bounded at the cap (no DoS)', () => {
  ...
  assert.equal(state.paste.active, true, 'paste is in progress after the start marker');

  // Feed far more than the cap, in chunks, with no end marker.
  for (let i = 0; i < 20; i += 1) {
    state = decode(new Uint8Array(1000).fill(0x41), state, opts).state;
    assert.ok(stateSize(state) <= FUZZ_PASTE_CAP + 1024, `state ${stateSize(state)} stayed bounded`);
  }
  assert.ok(state.paste.bytes.length <= FUZZ_PASTE_CAP, 'paste accumulation never exceeds the cap');
  assert.doesNotThrow(() => flush(state, opts));
});
