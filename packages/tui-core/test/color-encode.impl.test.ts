...
test('#rgb short form expands each nibble', () => {
  expect(encode('#abc', 'fg', 'truecolor')).toBe('\x1b[38;2;170;187;204m');
});
...
test('every attribute bit encodes to its SGR code', () => {
  ...
  for (const [bit, code] of cases) {
    expect(encodeStyle('default', 'default', bit, TRUECOLOR)).toBe(`\x1b[${code}m`);
  }
});

test('16-depth fg and bg role codes span the normal and bright ranges', () => {
  expect(encode('red', 'fg', '16')).toBe('\x1b[31m'); // normal fg 30+1
  expect(encode('red', 'bg', '16')).toBe('\x1b[41m'); // normal bg 40+1
  expect(encode('brightWhite', 'fg', '16')).toBe('\x1b[97m'); // bright fg 90+7
  expect(encode('brightWhite', 'bg', '16')).toBe('\x1b[107m'); // bright bg 100+7
});
